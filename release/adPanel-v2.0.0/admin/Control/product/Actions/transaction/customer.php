...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['import_customer'])) {
      ...
      if ($fileExt === 'csv') {
        ...
        if ($handle) {
          ...
          while (($data = fgetcsv($handle, 1000, ',')) !== FALSE) {
            ...
            if ($nama === '') {
              $importErrors[] = "Baris $rowNum: Nama customer kosong";
              continue;
            }
            ...
            $stmt = mysqli_prepare($kon, "INSERT INTO customer (nama, alamat, contact, phone, email, npwp) VALUES (?, ?, ?, ?, ?, ?)");
            if ($stmt) {
              mysqli_stmt_bind_param($stmt, 'ssssss', $nama, $alamat, $contact_val, $phone_val, $email_val, $npwp_val);
        ...
        for ($rowNum = 2; $rowNum <= $highestRow; $rowNum++) {
          ...
          $npwp = trim($worksheet->getCellByColumnAndRow(6, $rowNum)->getFormattedValue());
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_customer'])) {
  ...
  if ($nama === '') $errors[] = 'Nama customer wajib diisi.';
  ...
  if (empty($errors)) {
    ...
    if ($stmt) {
      ...
      if (mysqli_stmt_execute($stmt)) {
        ...
        $errors[] = 'Gagal menyimpan customer: ' . mysqli_stmt_error($stmt);
      }
      mysqli_stmt_close($stmt);
    } else {
...
<div class="card" style="margin-bottom:16px;">
  ...
  <form method="POST" style="margin-top:12px;">
    ...
    <div class="form-actions">
      <button class="btn btn-add" type="submit" name="save_customer">Simpan Customer</button>
  ...
  <?php if ($importSuccess > 0): ?>
    <div id="success-msg" class="card" style="position:fixed;top:20px;right:20px;z-index:9999;border-left:4px solid #27ae60;background:#fff;box-shadow:0 6px 18px rgba(15,23,42,0.18);width:320px;">
      <div style="color:#155724;padding:10px 12px">Berhasil import <?php echo $importSuccess; ?> customer.</div>
    </div>
    <script>setTimeout(() => { var el = document.getElementById("success-msg"); if(el) el.style.display = "none"; }, 5000);</script>
  <?php endif; ?>

  <?php if ($success): ?>
    <div id="success-msg" class="card" style="position:fixed;top:20px;right:20px;z-index:9999;border-left:4px solid #27ae60;background:#fff;box-shadow:0 6px 18px rgba(15,23,42,0.18);width:320px;">
      <div style="color:#155724;padding:10px 12px">Customer berhasil ditambahkan.</div>
    </div>
    <script>setTimeout(() => { var el = document.getElementById("success-msg"); if(el) el.style.display = "none"; }, 5000);</script>
  <?php endif; ?>
  ...
  <div class="card" style="margin-top:12px;border-left:4px solid #3498db;">
    ...
    <p style="margin:0 0 12px 0;color:#7f8c8d;font-size:14px;">Format kolom: Nama | Alamat | Contact | Phone | Email | NPWP</p>
    <form method="POST" enctype="multipart/form-data" style="display:flex;gap:8px;align-items:flex-end;flex-wrap:wrap;">
      ...
      <button class="btn btn-import" type="submit" name="import_customer">Import</button>
    </form>
  </div>
</div>
