...
function render_main_content($menu) {
  ...
  // Use includes for pages; pages that are included should support being embedded (IN_MENU_ADMIN)
    if ($menu === 'dashboard') {
      ...
      $cards = [
        ['href' => '?menu=product', 'title' => 'Product', 'desc' => 'Kelola data produk & stok', 'icon' => '📦'],
        ['href' => '?menu=itc_product', 'title' => 'Product Interchange', 'desc' => 'Relasi interchange produk', 'icon' => '🔄'],
        ['href' => '?menu=list_itc', 'title' => 'Part Number Interchange', 'desc' => 'Relasi interchange part number', 'icon' => '🔢'],
        ['href' => '?menu=banner_product', 'title' => 'Banner Product', 'desc' => 'Kelola banner utama', 'icon' => '🖼️'],
        ['href' => '?menu=banner_brand', 'title' => 'Banner Brand', 'desc' => 'Logo brand carousel', 'icon' => '🏷️'],
        ['href' => '?menu=transaction', 'title' => 'Transaction', 'desc' => 'Pantau transaksi', 'icon' => '💳'],
        ['href' => '?menu=report', 'title' => 'Reports', 'desc' => 'Laporan produk & aktivitas', 'icon' => '📊'],
      ];
      if (!empty($_SESSION['role']) && in_array($_SESSION['role'], ['B','C'])) {
        $cards[] = ['href' => '?menu=create_admin', 'title' => 'Tambah Admin', 'desc' => 'Kelola akun admin', 'icon' => '👤'];
      }
      echo '<div style="padding:1.5rem;">';
      echo '<h1 style="margin:0 0 8px;">Menu Admin</h1>';
      echo '<p style="margin:0 0 18px;color:#475569;">Pilih menu di bawah untuk membuka halaman terkait.</p>';
      echo '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:14px;">';
      foreach ($cards as $card) {
        echo '<a class="dashboard-card-link" href="' . htmlspecialchars($card['href']) . '" style="text-decoration:none;color:inherit;">';
        echo '<div style="background:#fff;border:1px solid #e5e7eb;border-radius:10px;padding:14px;box-shadow:0 4px 12px rgba(15,23,42,0.06);display:flex;flex-direction:column;gap:6px;">';
        echo '<div style="font-size:22px;">' . $card['icon'] . '</div>';
        echo '<div style="font-weight:700;font-size:1.05rem;">' . htmlspecialchars($card['title']) . '</div>';
        echo '<div style="color:#64748b;font-size:0.95rem;">' . htmlspecialchars($card['desc']) . '</div>';
        echo '</div>';
        echo '</a>';
      }
      echo '</div>';
      echo '<div style="margin-top:16px;color:#475569;font-size:0.95rem;">Total produk saat ini: <strong>' . $product_count . '</strong></div>';
      echo '</div>';
      return;
    }
