...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_trx'])) {
	...
	if (empty($trxErrors)) {
		...
		try {
			...
			$update = mysqli_prepare($kon, "UPDATE Product SET Qty = ? WHERE part_number = ?");
			if (!$update) {
				throw new RuntimeException('Prepare update Product gagal: ' . mysqli_error($kon));
			}
			mysqli_stmt_bind_param($update, 'is', $qtyAfter, $partNumber);
			if (!mysqli_stmt_execute($update)) {
				$err = mysqli_stmt_error($update);
				mysqli_stmt_close($update);
				throw new RuntimeException('Gagal update Qty: ' . $err);
			}
			mysqli_stmt_close($update);

			$insert = mysqli_prepare($kon, "INSERT INTO stock_transactions (part_number, qty_before, qty_in, qty_out, qty_after, note, created_at) VALUES (?, ?, ?, ?, ?, ?, NOW())");
			if (!$insert) {
				throw new RuntimeException('Prepare stock_transactions gagal: ' . mysqli_error($kon));
			}
...
(function(){
	...
	function updateNoteFromParty(){
		...
		var val = typeSelect.value;
		var note = '';
		if (val === 'in' && supplier && supplier.value.trim() !== '') {
			note = 'Supplier: ' + supplier.value.trim();
		} else if (val === 'out' && customer && customer.value.trim() !== '') {
			note = 'Customer: ' + customer.value.trim();
		}
	...
	<div class="table-wrapper">
		<table class="table" id="stock-table">
			...
			<tbody>
				...
				?>
				<tr data-part-number="<?php echo htmlspecialchars($row['part_number']); ?>" data-qty="<?php echo htmlspecialchars($row['Qty']); ?>">
	...
	<div class="table-wrapper history-table-wrapper">
		<table class="table">
			...
			<tbody>
				...
				<?php
				$historyRows = mysqli_query($kon, "SELECT id, part_number, qty_before, qty_in, qty_out, qty_after, note, created_at FROM stock_transactions ORDER BY created_at DESC LIMIT 100");
				if ($historyRows && mysqli_num_rows($historyRows) > 0):
					while ($trx = mysqli_fetch_assoc($historyRows)):
				?>
				<tr data-part-number="<?php echo htmlspecialchars($trx['part_number']); ?>">
