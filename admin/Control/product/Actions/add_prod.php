        ...
        if (empty($errors)) {
                ...
                try {
                        ...
                        if ($hasProductImageColumn && $primaryImageData !== null) {
                          $stmt = mysqli_prepare($kon, "INSERT INTO Product (part_number, itc, description, brand, category, Qty, UoM, berat, massa, image) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                          if (!$stmt) {
                            throw new RuntimeException('Prepare product gagal: ' . mysqli_error($kon));
                          }
                          mysqli_stmt_bind_param($stmt, "sssssisisb", $part_number, $itc, $description, $brand, $category, $qty, $uom, $brt, $massa, $primaryImageData);
                          mysqli_stmt_send_long_data($stmt, 9, $primaryImageData);
                        } else {
                          $stmt = mysqli_prepare($kon, "INSERT INTO Product (part_number, itc, description, brand, category, Qty, UoM, berat, massa) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
                          if (!$stmt) {
                            throw new RuntimeException('Prepare product gagal: ' . mysqli_error($kon));
                          }
                          mysqli_stmt_bind_param($stmt, "sssssisis", $part_number, $itc, $description, $brand, $category, $qty, $uom, $brt, $massa);
                        }
    ...
    if ($ext === 'csv') {
      if (($h = fopen($tmp, 'r')) !== false) {
        ...
        while (($r = fgetcsv($h, 0, ',')) !== false) {
          ...
          mysqli_stmt_bind_param($stmt, 'sssssisis', $pn, $itc, $desc, $brand, $cat, $qty, $uom, $brt, $massa);
          if (!mysqli_stmt_execute($stmt)) { 
...
<?php if (!empty($errors)): ?>
  ...
  <h2>Tambah Product</h2>
   <div class="container-fluid p-1 card-inner" style="overflow:hidden; background:#fff; box-shadow:0 2px 5px #2563eb22; height:700px;">
    <form method="POST" enctype="multipart/form-data" class="card form" style="height:700px; overflow:hidden;">
      <div class="row-two">
        <div class="col">
